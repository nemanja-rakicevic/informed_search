...
class ReacherOneShotEnv_v1(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def _get_obs(self):
        err = 100*(self.get_body_com("real_target") - self.get_body_com("target"))[:-1]
        return np.concatenate([
            self.sim.data.qpos.flat[:],       # joint0, joint1, joint2, joint3, joint4, ballx, bally, targetx, targety
            self.sim.data.qvel.flat[:5],      # velocities joint0, joint1, joint2, joint3, joint4
            err                                 # error distance: x,y
        ])