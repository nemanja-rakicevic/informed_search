...
Date  :         June 2020
Description:
                Load the model from a file and perform a full test
"""
...
def main_test(load_path, display=True):
    ...
    test_stats = experiment.evaluate_test_cases(num_trial=None, 
                                                save_model=False, 
    ...
    print("\n{} TESTING {} cases {}"
          ...
          "\n - Polar error norm mean:  {:4.2f}\n{}".format(
            '-'*15, num_total, '-'*15, num_success, num_fails, 
            num_total, model_error_mean, errors_mean[0],
            errors_mean[1], '-'*50))
