...
yticks = np.linspace(min(dim1[0], dim1[-1]), max(dim1[0], dim1[-1]), 4).round(2)
yticks1 = np.linspace(min(dim1[0], dim1[-1]), max(dim1[0], dim1[-1]), 5).round(2)

zticks_alpha = np.linspace(0, M_angle.max(), 5).round()
zticks_L = np.linspace(150, 350, 5).round()  
...
ax.set_ylabel('right dx', labelpad=15)
ax.set_xlabel('wrist angle', labelpad=25)
ax.set_zlabel('[degrees]', rotation='vertical', labelpad=8)
...
if M_angle.shape[0]>1:
    ax.plot_surface(X, Y, M_angle[0,3,:,3,:,4].reshape(len(dim1),len(dim2)), rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0, antialiased=False)
else:
    ax.plot_surface(X, Y, M_angle[0,0,:,0,:,0].reshape(len(dim1),len(dim2)), rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0, antialiased=False)
...
ax.set_ylabel('right dx', labelpad=15)
ax.set_xlabel('wrist angle', labelpad=25)
ax.set_zlabel('[cm]', rotation='vertical', labelpad=12)
ax.set_xticks(xticks)
ax.set_yticks(yticks)
...
if M_angle.shape[0]>1:
    ax.plot_surface(X, Y, M_dist[0,3,:,3,:,4].reshape(len(dim1),len(dim2)), rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0, antialiased=False)
else:
    ax.plot_surface(X, Y, M_dist[0,0,:,0,:,0].reshape(len(dim1),len(dim2)), rstride=1, cstride=1, cmap=cm.coolwarm, linewidth=0, antialiased=False)
...
# if M_angle.shape[0]>1:
#     ax.plot_surface(X, Y, (1-penal_PDF[0,3,:,3,:,4].reshape(len(dim1),len(dim2))), rstride=1, cstride=1, cmap=cm.copper, linewidth=0, antialiased=False)
# else:
...
# ax.tick_params(axis='z', direction='out', pad=7)
# if M_angle.shape[0]>1:
#     ax.plot_surface(X, Y, var_angle[0,3,:,3,:,4].reshape(len(dim1),len(dim2)), rstride=1, cstride=1, cmap=cm.winter, linewidth=0, antialiased=False)
# else:
#     ax.plot_surface(X, Y, var_angle[0,0,:,0,:,0].reshape(len(dim1),len(dim2)), rstride=1, cstride=1, cmap=cm.winter, linewidth=0, antialiased=False)
...
# SAVEFIG
pl.savefig("Fig_for_paper_M18.svg")
pl.show()
...
X, Y = np.meshgrid(dim2, dim1)
fig = pl.figure("DISTRIBUTIONs at step: "+str(tr), figsize=None)
fig.set_size_inches(fig.get_size_inches()[0]*2.5, fig.get_size_inches()[1]*2)

xticks = np.linspace(min(dim2[0], dim2[-1]), max(dim2[0], dim2[-1]), 6).round(2)
xticks1 = np.linspace(min(dim2[0], dim2[-1]), max(dim2[0], dim2[-1]), 5).round(2)

yticks = np.linspace(min(dim1[0], dim1[-1]), max(dim1[0], dim1[-1]), 4).round(2)
yticks1 = np.linspace(min(dim1[0], dim1[-1]), max(dim1[0], dim1[-1]), 4).round(2)

zticks_alpha = np.linspace(0, model.mu_alpha.max(), 5).round(1)
zticks_L = np.linspace(150, 350, 5).round(1)  
# zticks_alpha = np.linspace(0, model.mu_alpha.max(), 5).round(1)
# zticks_L = np.linspace(model.mu_L.min(), model.mu_L.max(), 5).round(1)     
zticks_unc = np.linspace(model.var_alpha.min(), model.var_alpha.max(), 5).round(2)   
zticks_sel = np.linspace(model.info_pdf.min(), model.info_pdf.max(), 5).round(3)    
...
ax.set_ylabel('right dx', labelpad=10)
ax.set_xlabel('wrist angle', labelpad=15)
ax.set_xticks(xticks1)
ax.set_yticks(yticks1)
ax.set_xticklabels([str(x) for x in xticks], rotation=48)
ax.set_yticklabels([str(x) for x in yticks], rotation=-18)
ax.tick_params(axis='x', direction='out', pad=-4)
ax.tick_params(axis='y', direction='out', pad=0)
ax.tick_params(axis='z', direction='out', pad=3)
if model.param_dims[0]>1:
...
ax.set_xticks(xticks1)
ax.set_yticks(yticks1)
# ax.set_zticks(zticks_sel)
ax.set_xticklabels([str(x) for x in xticks], rotation=48)
ax.set_yticklabels([str(x) for x in yticks], rotation=-18)
ax.set_zticklabels(['0.0','0.2','0.4','0.6','0.8'])
ax.tick_params(axis='x', direction='out', pad=-4)
ax.tick_params(axis='y', direction='out', pad=0)
ax.tick_params(axis='z', direction='out', pad=5)
if model.param_dims[0]>1:
